...
public class WeightedGraph{
  ...
  private class VertexForWeighted{
    ...
    private String name;
    // parent is the parent of the vertex and can be used to backtrace when searching
    ...
    // finalized is used to check whether the shortest distance from the starting vertex to this vertex is found
    private boolean finalized;
    // edgesSorted is used to certify that the list of edges connecting to the vertex has been sorted alphabetically when it is true
    ...
    /**
     * If the list of edges connecting to the vertex is not certified as having been sorted alphabetically, sort it alphabetically and set edgesSorted as true.
     */
  ...
  // WeightedEdge is the class for the weighted and directed edge connecting 2 vertices in the graph
  ...
  private boolean addNodeForWeighted(String name){
    ...
    // Create a new vertex with name specified in input; if vertices has not had vertex with that name, put that vertex into vertices; otherwise, return false
  ...
  /**
   * Add edge connecting vertex that has name from (specified in input) to vertex that has name to (specified in input) and has weight specified in input.
   * Time complexity: O(n) (n is number of vertices that vertex having name from (specified in input) directs to in the graph)
   */
  public boolean addWeightedEdge(String from, String to, int weight){
    ...
    // Add vertex with name from (specified in input) and vertex with name to (specified in input) to graph
    addNodeForWeighted(from);
    addNodeForWeighted(to);
    // start is used to store the vertex with name from (specified in input) in vertices
     ...
     * if not, create ending to store the end vertex with name to (specified in input), add new edge with ending and weight to list of edges connecting from start, set edgesSorted of start as false, return true;
  ...
  /**
   * Add edges connecting from vertex that has name from (specified in input) to vertices that have names in tolist and have corresponding weights in weightlist.
   * Time complexity: O(nk) (n is number of vertices that vertex having name from (specified in input) directs to in the graph; k is number of elements in tolist or weightlist)
   */
  public boolean addWeightedEdges(String from, String[] tolist, int[] weightlist){
    ...
    // Add edge connecting from vertex that has name from (specified in input) to each vertex that has non-null name in tolist and has corresponding weight in weightlist
  ...
  /**
   * Print the content of the graph with vertices (with corresponding weights) sorted in alphabetical order and neighbors that each vertex directs to and are sorted in alphabetical order.
   * Time complexity: worst: O((n^2)*(logn)) (n is number of vertices in graph); 
   * if all edges lists are certified as alphabetically sorted, time complexity: O(n^2) (n is number of vertices in graph)
   */
  public void printWeightedGraph(){
     ...
     * sort the list of neighbors that it directs to and are in alphabetical order if the list of its neighbors has not been certified as having been sorted in alphabetical order,
  ...
  /**
   * Read the file with name specified in input and create directed, weighted graph based on the content of that file.
  ...
  public WeightedGraph readWeighted(String filename){
    ...
    // file is used to store the File instance of file with filename specified in input
     ...
     * add edge connecting from vertex having name as the first element in the line to other vertices having names and weights as the remaining corresponding elements in the line
     */
    try{
  ...
  /**
   * Find the shortest path from vertex with name from to vertex with name to by using Dijkstra's algorithm.
   * Time complexity: O(ne+n^2) (n is number of vertices in graph; e is number of edges in graph)
   */
  public String[] shortestPath(String from, String to){
    // path is used to store the LinkedList representation of the path required by this method
    LinkedList<String> path = new LinkedList<String>();
    // startVertex is used to store the starting vertex
    VertexForWeighted startVertex = vertices.get(from);
    // If startVertex is in the vertices, follow the code in brackets right after this condition
    if(startVertex != null){
      /**
       * Set all vertices's finalized as false and all vertices's cost as maximum value of integer that Java allows
       */
      ...
      // Set startVertex's cost as 0 and startVertex's parent as null, create vertexPointer to backtrace later based on parent
      startVertex.cost = 0;
      startVertex.parent = null;
      VertexForWeighted vertexPointer = null;
      // This loop continues until minimalVertex is null or has cost being equal to maximum value of integer that Java allows or the to vertex has been finalized
      while(true){
        ...
        // When the vertex that is unfinalized and having cost lower than that of minimalVertex or minimalVertex is null, set minimalVertex as that vertex
        while(verticesIterator.hasNext()){
        ...
        // If minimalVertex is null or has cost being equal to maximum value of integer that Java allows, end the loop
        if(minimalVertex == null || minimalVertex.cost == Integer.MAX_VALUE){
          break;
        }
        // Otherwise, set minimalVertex's finalized as true; if name of minimalVertex is equal to to (specified in input), set vertexPointer as minimalVertex and end the loop
         ...
         * if that neighbor is unfinalized and has cost higher than the cost from starting vertex through minimalVertex to that neighbor,
         * set that neighbor's cost as the cost from starting vertex through minimalVertex to neighbor and that neighbor's parent as minimalVertex
         */
      ...
      // Backtrace vertexPointer by going from finalized vertex with name to (specified in input) to starting vertex based on parent, add name of vertexPointer to path's beginning in each iteration
