public class NumArrayList implements NumList {
  ...
  /**
   * Initialize array with initial capacity, throw exception if capacity is
   * negative
   * Time complexity: O(1)
   */
  ...
  /**
   * Return the number of elements in the list (number of elements that array
   * currently stores)
   * Time complexity: O(1)
   */
  public int size() {
  ...
  public void add(double value) {
    // If list's size is smaller than list's capacity, just add value after list's
    // currently last element
    // Time complexity: O(1)
    if (size() < capacity()) {
      array[size()] = value;
    }
    // Otherwise, create newArray with larger capacity, copy elements from array to
    // newArray, add value after list's currently last element in newArray,then
    ...
    size = size() + 1;
    // If list's size is higher than 1 and added element is smaller than its
    // previous element, set isIncreasinglySorted as false
  ...
  public void insert(int i, double value) {
    ...
    if (i >= 0) {
      ...
      if (i >= size()) {
        ...
        // If list's size is smaller than list's capacity, shift all elements from
        // element currently with index i to the right; new elemeent with index i is
        // value from input
        // Time complexity: O(n) (n is the list's size)
        if (size() < capacity()) {
        ...
        // Otherwise, create newArray with larger capacity with same elements from index
        // 0 to index (i - 1), elements with index i to list's end shifted to the right,
        // new elemeent with index i being value from input, then assign newArray to
        // array
        // Time complexity: O(n) (n is the list's size)
        else {
          double[] newArray = new double[capacity() + 10];
        ...
        size = size() + 1;
        // If i is 0, if inserted element is higher than its next element, set
        ...
        // If i positive, if inserted element is smaller than its previous element or
        // higher than its next element, set isIncreasinglySorted as false
        else {
  ...
  public void remove(int i) {
    // If i is smaller than list's size and non-negative, shift all elements after
    // element with index i to the left
    // Time complexity: O(n) (n is the list's size)
    if (i >= 0 && i < size()) {
    ...
    // If i is higher than or equal to list's size, do nothing, so time complexity
    // for this case is O(1)
  }
  ...
  public boolean contains(double value) {
    // If an element in the list is equal to input value, the list contains input
    // value, so return true
    for (int i = 0; i < size(); i = i + 1) {
      if (value == array[i]) {
        return true;
      }
    }
    // After iterating through the list and no elements in the list are equal to
    // input value, the list does not contain input value, so return false
    return false;
  }
  ...
  public double lookup(int i) {
    // If i is smaller than list's size and non-negative, required element is
    // element with index i of array; do nothing if i is equal to or higher than
    // list's size
    // Time complexity: O(1)
    if (i < size() && i >= 0) {
      return array[i];
   ...
   * If otherList is in NumLinkedList, Time complexity: O(n^2) (n is the list's
   * size)
   * If otherList is null or lists' sizes are not equal, time complexity: O(1)
   */
  public boolean equals(NumList otherList) {
    // Check if otherList is not null and list's size is equal to size of otherList;
    // return false if at least one of these conditions fails
    // If otherList is in NumArrayList, time complexity: O(n) (n is the list's size)
    // If otherList is in NumLinkedList, Time complexity: O(n^2) (n is the list's
    // size)
    if (otherList != null && size() == otherList.size()) {
      // Compare each pair of elements having same indices; return false if pair's
      // elements are not equal to each other; return true after perceiving that all
      // pairs' elements are equal to each other
      for (int i = 0; i < size(); i = i + 1) {
  ...
  public void removeDuplicates() {
    // removed is used to check if examined element has been removed when iterating
    // through the list
    boolean removed = false;
    // Iterate through the list
    for (int i = 0; i < size(); i = i + 1) {
      // In each iteration, set removed to false, then remove examined element only if
      // it is equal to at least one of its left elements and examined element has not
      // been removed (remove the duplicate); removed is set to true after removing
      removed = false;
      ...
      // After removing the duplicate, elements after removed element are shift to the
      // left, so i needs to be decreased by 1 to contine with the next element after
      // it is shifted
      if (removed) {
        i = i - 1;
      }
  ...
  public String toString() {
    // If list is not empty, append elements to variable builder of StringBuilder
    // type, append spaces where necessary, return builder's string representation
    // Time complexity: O(n) (n is the list's size)
    if (size() > 0) {
  ...
  private boolean checkWhetherSorted() {
    // Iterate through the list; in each iteration, if the examined element is
    // smaller than or equal to its previous element, return false
    for (int i = 1; i < size(); i = i + 1) {
      if (array[i] < array[i - 1]) {
        return false;
      }
    }
    // If list's size is smaller than 2 or all examined elements are higher than
    // their previous elements, return true
   ...
   */
  public void reverse() {
    // Use a loop with integer i going from 0 to the highest integer that is lower
    // than size() / 2
    for (int i = 0; i < (size() / 2); i = i + 1) {
      // Swap element with index i with element with index (size() - 1 - i) in each
